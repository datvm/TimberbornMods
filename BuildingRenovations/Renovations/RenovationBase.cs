...
public abstract class RenovationBase
{
    ...
    public IEnumerable<GoodAmount> Cost => Spec.CustomCodeCost
        ? ((ICustomCostRenovation)this).GetCost()
        : Spec.ParsedCost;

    public virtual bool IsAvailableToThisGame(RenovationRegistry registry)
        => Spec.Cost is { Length: 0 } cost || registry.ContainsAllGoods(cost);
