...
namespace ConfigurablePlants.Patches;
...
public static class AreaPickerPatches
{
    ...
    {
        if (!MSettings.WithoutGroundValue) { return true; }

        __result = __instance._areaSelectionController.ProcessInput(
            (start, end, _) =>
            {
                previewCallback(NoGroundPlantTerrainService.ModifiedGetTerrainBlocks(start, end, __instance), start);
            },
            (start, end, _) =>
            {
                var blocks = NoGroundPlantTerrainService.ModifiedGetTerrainBlocks(start, end, __instance);
    ...
    {
        if (MSettings.NoConUpValue) { return; }
    ...
    {
        if (__result || !MSettings.WithoutGroundValue) { return; }
    ...
    {
        if (!MSettings.WithoutGroundValue) { return; }
