...
) : BaseComponent, IInitializableEntity, IUnfinishedStateListener, IDeletableEntity, IMaterialCraneJob
{
    ...
    public float Progress => ConstructionSite ? ConstructionSite.BuildTimeProgress : 0f;

    public string JobNameLoc => "LV.Cr.JobBuild";
    ...
    public IEnumerable<GoodAmount> GetTotalMaterials()
    {
        if (!ConstructionSite || !ConstructionSite.Inventory)
        {
            yield break;
        ...
        foreach (var allowed in ConstructionSite.Inventory.AllowedGoods)
        {
            yield return new GoodAmount(allowed.StorableGood.GoodId, allowed.Amount);
        }
    }
