...
public class EditDifficultyDialog(VisualElementLoader veLoader, PanelStack panelStack, ILoc t, CustomNewGameModeController customNewGameModeController) : DialogBoxElement
{
    ...
    public NewGameMode NewGameMode { get; private set; } = null!;

    public void SetContent(FactionSpec factionSpec, string mapName, NewGameMode current)
    {
        ...
        var newGamePanel = new NewGameModePanel(veLoader, null, panelStack, t, customNewGameModeController);
        newGamePanel.Load();

        newGamePanel.SelectFactionAndMap(
            factionSpec,
            // only map name is accessed:
            new(default, mapName, null, null, false, false, false, null));
