...
### Base classes (declared in `ModdableTimberborn.UpdatableEntityStats`)
...
`AvatarStat` is slightly special: `CanTrack` delegates to `EntityBadgeService` and returns `true` only when a highest-priority badge exists, while `TryGetTracker` currently succeeds for any non-null entity component.

`PopulationStat` is declared in the root namespace (`ModdableTimberborn.UpdatableEntityStats`) but its file is located in `UpdatableEntityStats/Implementations/PopulationStat.cs` because it is directly exposed through `UpdatableEntityStatService.PopulationStat`; it also provides a `GetGlobalTracker(options)` factory method for non-entity-bound global population tracking.
