...
public class PackagerManufactorySpec : BaseComponent
{
    [SerializeField]
...
public class PackagerPrefabProvider : IAssetProvider
{
    ...
    bool TryLoadPrefab(string path, out OrderedAsset orderedAsset)
    {
        ...
        var isPackager = IsPackagerPrefabName(name);
        if (!isPackager && !IsUnpackagerPrefabName(name))
        {
            return false;
        }

        var faction = name.Split('.')[1];
        var prefabName = (isPackager ? PackagerName : UnpackagerName) + "." + faction.ToPascalCase();
        
        orderedAsset = new(0, MakePrefab(isPackager, prefabName));
        return true;
    }
    ...
    static GameObject MakePrefab(bool isPackager, string prefabName)
    {
        ...
        obj.AddComponent<ManufactorySpec>();
    ...
    public static bool IsPackagerPrefabName(string prefabName) => prefabName.StartsWith(PackagerName + ".", StringComparison.OrdinalIgnoreCase);
    public static bool IsUnpackagerPrefabName(string prefabName) => prefabName.StartsWith(UnpackagerName + ".", StringComparison.OrdinalIgnoreCase);
